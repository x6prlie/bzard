...
X11FullscreenDetectorPrivate::X11FullscreenDetectorPrivate()
	...
	if (!disp) {
		throw std::runtime_error{"X11FullscreenDetector: Cannot open display."};
	}
		...
		if (wm_state_prop) {
			...
			for (auto &a : as) {
				if (a == WM_STATE_FULLSCREEN) {
					...
					if (win_desktop_o && *win_desktop_o == current_desktop) {
						return true;
					}
				}
			}
...
X11FullscreenDetectorPrivate::getProperty(Window win, Atom expectedType,
	...
	if (XGetWindowProperty(disp, win, propertyAtom, 0, MAX_PROPERTY_VALUE_LEN,
	                       False, expectedType, &prop_type, &prop_format,
	                       &items_count, &prop_bytes_after,
	                       &prop_ptr) != Success) {
		std::cerr << "X11FullscreenDetector: can't get property" << std::endl;
		return {};
	}
	if (prop_ptr == nullptr) {
...
std::vector<Window> X11FullscreenDetectorPrivate::getClientList() const {
	...
	if (!client_list)
		throw std::runtime_error{"X11FullscreenDetector: cannot get windows."};
